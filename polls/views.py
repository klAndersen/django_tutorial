"""
Taken from: https://docs.djangoproject.com/en/1.8/intro/tutorial04/#amend-views

See also: https://docs.djangoproject.com/en/1.8/topics/class-based-views/
...
specific template name instead of the auto-generated default template name. We also specify the template_name for the
...
template_name to tell ListView to use our existing "polls/index.html" template.

In previous parts of the tutorial, the templates have been provided with a context that contains the question and 
latest_question_list context variables. For DetailView the question variable is provided automatically - since we're 
using a Django model (Question), Django is able to determine an appropriate name for the context variable. However, 
for ListView, the automatically generated context variable is question_list. To override this we provide the 
context_object_name attribute, specifying that we want to use latest_question_list instead. As an alternative approach, 
you could change your templates to match the new default context variables - but it's a lot easier to just tell Django 
